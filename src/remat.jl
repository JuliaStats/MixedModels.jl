...
- `scratch`: a `Matrix{T}`
- `covstruct`: the [`CovarianceStructure`](@ref) parameterizing `λ` in terms of `θ`
"""
mutable struct ReMat{T,S} <: AbstractReMat{T}
    ...
    # at which point wtz is replaced by a new matrix
    trm::Any
    refs::Vector{Int32}
    levels::Any
    ...
    scratch::Matrix{T}
    covstruct::CovarianceStructure{T}
end

# maintain backwards compatibility with the field set before the
# introduction of covariance structures
function ReMat{T,S}(
    trm, refs, levels, cnames, z, wtz, λ, inds, adjA, scratch
) where {T,S}
    return ReMat{T,S}(
        trm, refs, levels, cnames, z, wtz, λ, inds, adjA, scratch, Unstructured{T}()
    )
end
...
function _amalgamate(reterms::Vector, T::Type)
    ...
    for (f, inds) in factordict
        if isone(length(inds))
            ...
            trms = reterms[inds]
            all(rt -> rt.covstruct isa Unstructured, trms) || throw(
                ArgumentError(
                    "random-effects terms with structured covariance cannot share " *
                    "a grouping factor with other random-effects terms",
                ),
            )
            trm1 = first(trms)
            trm = trm1.trm
...
Overwrite `v` with the current value of `θ` for `A`.

For an [`Unstructured`](@ref) covariance structure these are the elements of the
blocks in the lower triangle of `A.Λ` (column-major ordering); otherwise the
`θ` stored in the covariance structure.
"""
function getθ!(v::AbstractVector, A::ReMat{T}) where {T}
    return getθ!(v, A, A.covstruct)
end

function getθ!(v::AbstractVector, A::ReMat{T}, ::Unstructured) where {T}
...
function getθ!(v::AbstractVector, A::ReMat{T}, cs::CovarianceStructure{T}) where {T}
    length(v) == length(cs.θ) || throw(DimensionMismatch("length(v) ≠ nθ(A)"))
    return copyto!(v, cs.θ)
end
...
"""
nθ(A::ReMat) = _nθ(A.covstruct, A)

_nθ(::Unstructured, A::ReMat) = length(A.inds)
_nθ(cs::CovarianceStructure, ::ReMat) = length(cs.θ)

"""
    lowerbd{T}(A::ReMat{T})

Return the vector of _canonical_ lower bounds on the parameters `θ` associated with `A`.

Note that optimization is performed without constraints, followed by
canonicalization of `θ` into the region described by these bounds.
"""
lowerbd(A::ReMat) = _lowerbd(A.covstruct, A)

function _lowerbd(::Unstructured, A::ReMat{T}) where {T}
    k = size(A.λ, 1)  # construct diagind(A.λ) by hand following julia#52115
    dind = range(1; step=k + 1, length=k)
    return T[ind in dind ? zero(T) : T(-Inf) for ind in A.inds]
end

_lowerbd(cs::CovarianceStructure, ::ReMat) = lowerbd(cs)

"""
    initialθ(A::ReMat{T})

Return the initial value of the parameters `θ` associated with `A`, chosen
such that `λ` is the identity.
"""
initialθ(A::ReMat) = _initialθ(A.covstruct, A)

function _initialθ(::Unstructured, A::ReMat{T}) where {T}
    k = size(A.λ, 1)
    dind = range(1; step=k + 1, length=k)
    return T[ind in dind ? one(T) : zero(T) for ind in A.inds]
end

_initialθ(cs::CovarianceStructure, ::ReMat) = initialθ(cs)
...
function LinearAlgebra.copy_oftype(A::ReMat{<:Any,S}, ::Type{T}) where {T,S}
    return ReMat{T,S}(
        ...
        LinearAlgebra.copy_oftype(A.covstruct, T),
    )
end

function setθ!(A::ReMat{T}, v::AbstractVector) where {T}
    return setθ!(A, A.covstruct, v)
end

function setθ!(A::ReMat{T}, ::Unstructured, v::AbstractVector) where {T}
    # linear indexing into the wrapper (not its parent) because for a
    # Diagonal λ the `inds` are indices into the full matrix
    A.λ[A.inds] = v
    return A
end

function setθ!(A::ReMat{T}, cs::CovarianceStructure{T}, v::AbstractVector) where {T}
    copyto!(cs.θ, v)
    updateλ!(cs, A.λ, cs.θ)
    return A
end
...
Canonicalize the parameters `θ` of `A` without changing the implied covariance
matrix `λλ'`, following unconstrained optimization.
"""
rectify!(A::ReMat) = _rectify!(A.covstruct, A)

function _rectify!(::Unstructured, A::ReMat)
    rectify!(A.λ)
    return A
end

function _rectify!(cs::CovarianceStructure, A::ReMat)
    canonicalize!(cs)
    updateλ!(cs, A.λ, cs.θ)
    return A
end
...
Impose the covariance structure tagged by `C` on `A`, replacing `λ`, `inds`
and `covstruct` as necessary and initializing `θ` such that `λ` is the
identity.
"""
structure!(A::ReMat, ::Type{ZeroCorrStruct}) = zerocorr!(A)

# for a scalar term a scaled identity is exactly the (unstructured) single
# variance component, so leave it as-is rather than converting the 1×1 λ to a
# Diagonal (which would break the ReMat{T,1} specializations)
structure!(A::ReMat{T,1}, ::Type{ScaledIdentity}) where {T} = A

function structure!(A::ReMat{T,S}, ::Type{ScaledIdentity}) where {T,S}
    cs = ScaledIdentity{T}(S)
    A.λ = Diagonal(A.λ)
    A.inds = collect(range(1; step=S + 1, length=S))
    A.covstruct = cs
    updateλ!(cs, A.λ, cs.θ)
    return A
end

function structure!(A::ReMat{T,S}, ::Type{C}) where {T,S,C<:CompoundSymmetry}
    S > 1 || throw(
        ArgumentError(
            "compound symmetry requires at least two random effects per level; " *
            "drop the wrapper for scalar random-effects terms",
        ),
    )
    cs = C{T}(S)
    A.λ = LowerTriangular(Matrix{T}(I, S, S))
    m = reshape(1:abs2(S), (S, S))
    A.inds = [m[i, j] for j in 1:S for i in j:S]
    A.covstruct = cs
    updateλ!(cs, A.λ, cs.θ)
    return A
end
