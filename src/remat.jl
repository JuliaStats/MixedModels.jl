...
Base.adjoint(A::ReMat) = Adjoint(A)

Base.getindex(A::ReMat, i::Integer, j::Integer) = getindex(A.adjA, j, i)

Base.length(A::ReMat) = length(A.adjA)

Base.size(A::ReMat) = reverse(size(A.adjA))

Base.size(adjA::Adjoint{<:Any,ReMat}) = size(A.adjA)

function Base.size(A::ReMat, i::Integer)
    i > 0 || throw(ArgumentError("i must be > 0"))
    adjA = A.adjA
    if i == 1
        adjA.n
    elseif i == 2
