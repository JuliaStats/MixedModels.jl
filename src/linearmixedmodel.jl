...
LinearMixedModel(f::FormulaTerm, tbl; kwargs...) =
    LinearMixedModel(f::FormulaTerm, Tables.columntable(tbl); kwargs...)
...
function LinearMixedModel(
    f::FormulaTerm, tbl::Tables.ColumnTable; contrasts=Dict{Symbol,Any}(), wts=[], σ=nothing, kwargs...)
    ...
    return LinearMixedModel(y, Xs, form, wts, σ; kwargs...)
end
...
function LinearMixedModel(
    ...
    return LinearMixedModel(convert(Array{T}, y), only(feterms), reterms, form, wts, σ; kwargs...)
end
...
function LinearMixedModel(
    ...
    if amalgamate && length(reterms) > 1
        # have to make sure we grab the function and not the local variable
...
function StatsAPI.fit(
    ...
    return if !isempty(offset)
        ...
        fit(LinearMixedModel, f, tbl; wts, contrasts, progress, REML, σ, thin, amalgamate)
    end
end
