...
function createAL(allterms::Vector{Union{AbstractReMat{T},FeMat{T}}}) where {T}
    ...
    for i = 2:nretrm      # check for fill-in due to non-nested grouping factors
        ...
        for j = 1:(i-1)
            ...
            if !isnested(cj, ci)
                for l = i:k
                    L[Block(l, i)] = Matrix(view(L, Block(l, i)))
                end
                break
            end
        end
...
function ranef!(
    ...
    for j = 1:k
        mul!(
            vec( copyto!( v[j], view(L, Block(length(m.allterms), j)) ) ),
            view(L, Block(k+1, j))',
            β,
            -one(T),
            one(T),
    ...
    for i = k:-1:1
        Lii = view(L, Block(i, i))
        vi = vec(v[i])
        ldiv!(adjoint(isa(Lii, Diagonal) ? Lii : LowerTriangular(Lii)), vi)
        for j = 1:(i-1)
            mul!(vec(v[j]), view(L, Block(i, j))', vi, -one(T), one(T))
        end
    end
    if !uscale
...
function reevaluateAend!(m::LinearMixedModel)
    ...
    for (j, trm) in enumerate(m.allterms)
        mul!(view(A, Block(nblk, j)), trmn', trm)
    end
    m
end
...
function updateA!(m::LinearMixedModel)
    ...
    for j = 1:k
        for i = j:k
            mul!(view(A, Block(i, j)), allterms[i]', allterms[j])
        end
    end
    m
end
...
function updateL!(m::LinearMixedModel{T}) where {T}
    ...
    for j = 1:k                         # copy lower triangle of A to L
        for i = j:k
            copyto!(view(L, Block(i, j)),
                    view(A, Block(i, j)))
        end
    end
    for (j, cj) in enumerate(m.reterms)  # pre- and post-multiply by Λ, add I to diagonal
        scaleinflate!(view(L, Block(j, j)), cj)
        for i = (j+1):k         # postmultiply column by Λ
            rmulΛ!(view(L, Block(i, j)), cj)
        end
        for jj = 1:(j-1)        # premultiply row by Λ'
            lmulΛ!(cj', view(L, Block(j, jj)) )
        end
    end
    for j = 1:k                         # blocked Cholesky
        Ljj = view(L, Block(j, j))
        for jj = 1:(j-1)
            rankUpdate!(Hermitian(Ljj, :L), view(L, Block(j, jj)), -one(T), one(T))
        end
        cholUnblocked!(Ljj, Val{:L})
        LjjT = isa(Ljj, Diagonal) ? Ljj : LowerTriangular(Ljj)
        for i = (j+1):k
            Lij = view(L, Block(i, j))
            for jj = 1:(j-1)
                mul!(Lij, view(L, Block(i, jj)), view(L, Block(j, jj))', -one(T), one(T))
            end
            rdiv!(Lij, LjjT')
        end
    end
