...
function MixedModels.rankUpdate!(
    ...
    @inbounds for idx in eachindex(Cdiag, Adiag)
        Cdiag[idx] = fma(β,Cdiag[idx], α * abs2(Adiag[idx]))
    end
    return C
end
...
function _columndot(rv, nz, rngi, rngj)
    ...
    while i ≤ ni && j ≤ nj
        ...
        if ri == rj
            @inbounds accum = fma(nz[rngi[i]], nz[rngj[j]], accum)
            i += 1
            j += 1
        elseif ri < rj
            i += 1
    ...
    if lower
        @inbounds for jj in axes(A, 2)
            ...
            for (k, j) in enumerate(rangejj)
                ...
                for i in k:lenrngjj
                    kk = rangejj[i]
                    Cd[rv[kk], rvj] = fma(nz[kk], anzj, Cd[rv[kk], rvj])
                end
            end
        end
    else
        @inbounds for j in axes(C, 2)
            rngj = nzrange(A, j)
            for i in 1:(j - 1)
                Cd[i, j] = fma(α, _columndot(rv, nz, nzrange(A, i), rngj), Cd[i, j])
            end
            Cd[j, j] = fma(α, sum(i -> abs2(nz[i]), rngj), Cd[j, j])
        end
    end
    return C
end
...
function rankUpdate!(
    ...
    @inbounds for i in eachindex(Cdiag)
        Cdiag[i] = fma(α, sum(abs2, view(A, i, :)), Cdiag[i])
    end
    ...
    for k in axes(Cdat, 3)
        ...
        for i in axes(Cdat, 1), j in 1:i
            ...
            for idx in axes(A, 2)
                ...
                AtAij = fma(A[iind, idx], A[jind, idx], AtAij)
            end
            Cdat[i, j, k] = fma(α, AtAij, Cdat[i, j, k])
        end
    end
