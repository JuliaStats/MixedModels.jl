...
==============================
- Support for multimembership random effects, where an observation may be associated with several (weighted) levels of a grouping factor. Memberships are specified via the new `memberships` keyword argument to the model constructors and `fit`, as a mapping from grouping-variable name to a weights matrix; the grouping variable need not be a column in the data. New exports: `MembershipMatrix`, `membershipmatrix`, `interactionweights`, and `MultimembershipReMat`. `predict` and `simulate!` with new data take a `memberships` keyword argument for the new observations. Limitations: `nAGQ > 1`, `leverage`, and `cooksdistance` are not supported for models with multimembership terms, and the diagonal block of the Cholesky factor for such a term is dense (`O(q²)` storage, `O(q³)` factorization time in the number of levels `q`). [#330], [#797]
...
[#330]: https://github.com/JuliaStats/MixedModels.jl/issues/330
...
[#797]: https://github.com/JuliaStats/MixedModels.jl/issues/797
