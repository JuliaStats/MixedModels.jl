...
==============================
- GLMMs for families with a dispersion parameter (`Gamma`, `InverseGaussian`, and `Normal` with a non-identity link) are now fitted correctly.  How ϕ is estimated depends on `fast`:
  - `fast=true` plugs in the Pearson moment estimator `ϕ̂ = pwrss(m) / nobs(m)`, which is the estimator lme4's `sigma()` uses.
  - `fast=false` (the default) treats ϕ as a parameter of the outer optimization alongside β and θ, so it converges to the conditional MLE.  For `Normal` the two coincide, since there the moment estimator *is* the MLE; for `Gamma` and `InverseGaussian` the MLE solves a digamma equation and the two answers differ.  ϕ is optimized on the log scale, so `optsum.final` for such a fit is `[β; θ; log ϕ]` and carries one more element than before.
- ϕ can now be fixed a priori for a GLMM, via the `σ` keyword argument to `GeneralizedLinearMixedModel`, giving `ϕ = σ²`.  This is the same mechanism `LinearMixedModel` already used for a known residual standard deviation: the value lives in `optsum.sigma`, and when it is set ϕ is not estimated and does not appear in the parameter vector.  Passing `σ` for a family without a dispersion parameter is an `ArgumentError`.  Note that, as for `LinearMixedModel` with a fixed `σ`, `dof` still counts the dispersion parameter.
- The random-effects penalty in the Laplace/AGQ objective is now `sum(u²)/ϕ` rather than `sum(u²)`.  θ is *relative* to the scale parameter in this package (`Var(b) = ϕΛΛ'`, matching the `σ` factor that `σs` and `VarCorr` already applied), so the penalty is on the ϕ scale too.  Specializing the corrected expression to a `Normal` response with an identity link now reproduces `objective(::LinearMixedModel)` exactly; the previous expression did not, and consequently reported both θ̂ and ϕ̂ incorrectly for dispersion families.
- `deviance`, `loglikelihood`, and `dispersion` now all read ϕ from one place, so they are mutually consistent under either estimator.  Previously `loglikelihood` used a `deviance(r) / sum(wts)` heuristic that did not agree with the quantity being optimized.
- Both the Laplace approximation (`nAGQ=1`) and adaptive Gauss-Hermite quadrature (`nAGQ>1`) handle the dispersion parameter; for AGQ the quadrature nodes are also spaced on the ϕ scale.  For families without a dispersion parameter, both objectives are unchanged, down to the last bit.
- The warning previously emitted when *constructing* a `GeneralizedLinearMixedModel` for a dispersion family has been removed.  Neither construction nor fitting emits anything now; instead `show` reports which of the two estimators produced the ϕ of the fitted model.  Putting it in `show` rather than in `fit!` keeps it available for as long as the model is, and stops it repeating once per replicate when `parametricbootstrap` refits in a loop.
- `GeneralizedLinearMixedModel` has gained a `ϕ` field, a zero- or one-element vector holding the free dispersion parameter when there is one.  This changes the positional argument list of the (internal) default constructor.
- `simulate`, `simulate!`, and hence `parametricbootstrap` now support families with a dispersion parameter, where they previously threw `ArgumentError("Families with a dispersion parameter not yet supported")`.  Responses are drawn from `Normal(μ, σ)`, `Gamma(1/ϕ, μϕ)`, or `InverseGaussian(μ, 1/ϕ)` as appropriate, these being the parameterizations whose variances match the variance function GLM.jl uses for the family.
- Relatedly, the simulated random effects are now scaled by σ on the GLMM path, as they already were for `LinearMixedModel`.  `b = σΛu` with `u ~ N(0, I)` is required by the same relative parameterization as above; previously the GLMM path drew `b = Λu` regardless of family, which for a dispersion family produced random effects too large by a factor of `1/σ`.
