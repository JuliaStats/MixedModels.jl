...
fm1 = lmer(:(Yield ~ 1|Batch), ds);
println(fm1)

ds2 = data("lme4", "Dyestuff2")
fm1a = lmer(:(Yield ~ 1|Batch), ds2);
println(fm1a)

psts = data("lme4", "Pastes")
fm2 = lmer(:(strength ~ (1|sample) + (1|batch)), psts);
println(fm2)

pen = data("lme4", "Penicillin")
fm3 = lmer(:(diameter ~ (1|plate) + (1|sample)), pen);
println(fm3)

chem = data("mlmRev", "Chem97")
fm4 = lmer(:(score ~ (1|school) + (1|lea)), chem);
println(fm4)

fm5 = lmer(:(score ~ gcsecnt + (1|school) + (1|lea)), chem);
println(fm5)
...
@time fm7 = lmer(:(y ~ dept*service + (1|s) + (1|d)), inst);
println(fm7)

sleep = data("lme4", "sleepstudy")
@time fm8 = lmer(:(Reaction ~ Days + (Days | Subject)), sleep);
println(fm8)
