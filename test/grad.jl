...
@testset "gradient" begin
    @testset "single_scalar" begin
        ...
        θ = fm1.θ
        blks = initialize_blocks!(grad_blocks(fm1), fm1, 1, 1, 1, 1)
        dblk = blks[1,1].diag
        ...
        θ = fm2.θ
        blks = initialize_blocks!(grad_blocks(fm2), fm2, 1, 1, 1, 1)
        @test all(≈(4. * only(θ)), blks[1, 1].diag)
        initialize_blocks!(blks, updateL!(setθ!(fm2, ones(1))), 1, 1, 1, 1)
        @test all(==(4.), blks[1, 1].diag)
        updateL!(setθ!(fm2, θ))

        fm3 = last(models(:pastes))     # first of two nested, scalar r.e. terms
        θ = fm3.θ
        blks = initialize_blocks!(grad_blocks(fm3), fm3, 1, 1, 1, 1)
        @test all(≈(4. * first(θ)), blks[1,1].diag)

        fm4 = only(models(:penicillin))
        blks = initialize_blocks!(grad_blocks(fm4), fm4, 1, 1, 1, 1)
        @test all(≈(12. * first(fm4.θ)), blks[1,1].diag)

        fm5 = first(models(:sleepstudy))
        blks = initialize_blocks!(grad_blocks(fm5), fm5, 1, 1, 1, 1)
        @test all(≈(20. * only(fm5.θ)), blks[1, 1].diag)
    end
    ...
    @testset "single_vector" begin
        ...
        θ = fm6.θ
        blks = initialize_blocks!(grad_blocks(fm6), fm6, 1, 1, 1, 2)
        blk_dat = blks[1,1].data
        ...
        ldiv!(LowerTriangular(first(fm6.L)), blks[1,1])

        initialize_blocks!(blks, fm6, 1, 2, 2, 2)
