...
fmre = LinearMixedModel(
    @formula(rt_trunc ~ 1 + (0 + spkr | subj) + (1 + load | item)),
...
@testset "markdown" begin
    ...
    gm3.optsum.feval = -1
    @test_logs (:warn, "Model has not been fit: results will be nonsense") sprint(
        show, mime, gm3
    )
    gm3.optsum.feval = 1
    @testset "lmm" begin
...
"""
    ...
    @test startswith(
        sprint(show, MIME("text/latex"), gm3),
        """
\\begin{tabular}
{l | r | r | r | r | r | r}
