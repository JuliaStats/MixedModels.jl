...
@testset "bootstrap" begin
    ...
    cov = shortestcovint(shuffle(1.0:100.0))
    ...
    @testset "save and restore replicates" begin
        ...
        @test restorereplicates(seekstart(io), m1, Float32) ≈
            restorereplicates(seekstart(io), m1, Float32)
        ...
        @test restorereplicates(seekstart(io), m1, MixedModelBootstrap{Float16}) ==
            restorereplicates(seekstart(io), m1, Float16)
    ...
    @testset "Rank deficient" begin
        ...
        @testset "$family" for family in [Normal(), Bernoulli()]
            model = @suppress fit(
                MixedModel, @formula(y ~ x + x2 + (1 | z)), data, family; progress=false
            )
