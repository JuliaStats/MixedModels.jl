...
@testset "scalarReMat" begin
    ...
    @testset "products" begin
        ...
        @test mul!(Array{Float64}(undef, (size(sf1, 2), size(sf2, 2))), sf1', sf2) ==
            Array(sf1'sf2)
        ...
        @test crp[6, 6] == 5
        @test size(crp) == (6, 6)
        @test crp.diag == fill(5.0, 6)
        rhs = y1'sf
        @test rhs == reshape([7525.0, 7640.0, 7820.0, 7490.0, 8000.0, 7350.0], (1, 6))
        @test ldiv!(crp, copy(rhs)') == [1505.0, 1528.0, 1564.0, 1498.0, 1600.0, 1470.0]
...
@testset "random effects term syntax" begin
    ...
    @testset "nesting" begin
        ...
        @test modelcols(f2.rhs, d2) ==
            [ones(20) d2.b .== :Y (d2.b .== :X) .* d2.a (d2.b .== :Y) .* d2.a]
        @test coefnames(f2.rhs) == ["(Intercept)", "b: Y", "b: X & a", "b: Y & a"]

        # check promotion
        f3 = apply_schema(@formula(0 ~ 0 + b / a), schema(d2), MixedModel)
        @test modelcols(f3.rhs, d2) ==
            [d2.b .== :X d2.b .== :Y (d2.b .== :X) .* d2.a (d2.b .== :Y) .* d2.a]
        ...
        psts = dataset("pastes")
        m = fit(
            MixedModel, @formula(strength ~ 1 + (1 | batch / cask)), psts; progress=false
        )
        m2 = fit(
            MixedModel,
        ...
        m2r = fit(
            MixedModel,
            term(:strength) ~
                term(1) + (term(1) | term(:batch)) + (term(1) | term(:batch) & term(:cask)),
            psts;
            progress=false,
        )
