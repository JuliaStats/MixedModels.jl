...
@testset "Dyestuff" begin
    ...
    @test objective(fm1) ≈ 327.32705988113764 atol=0.001
    @test fm1.θ ≈ [0.7525796976298461] atol=1.e-5
    @test fm1.λ ≈ [LowerTriangular(reshape(fm1.θ, (1,1)))] atol=1.e-5
    @test deviance(fm1) ≈ 327.32705988113764 atol=0.001
    @test aic(fm1) ≈ 333.32705988113764 atol=0.001
    @test bic(fm1) ≈ 337.53065202612413 atol=0.001
    @test fixef(fm1) ≈ [1527.5]
    @test dispersion_parameter(fm1)
    @test first(first(fm1.σs)) ≈ 37.26030335139678 atol=0.0001
    @test fm1.β ≈ [1527.5]
    @test dof(fm1) == 3
    @test nobs(fm1) == 30
    ...
    @test first(leverage(fm1)) ≈ 0.156505260370222 rtol=1.e-5
    @test sum(leverage(fm1)) ≈ 4.695157811106662 rtol=1.e-5
    cm = coeftable(fm1)
    ...
    @test logdet(fm1) ≈ 8.060134842137465 atol=0.001
    @test varest(fm1) ≈ 2451.2509577364976 atol=0.001
    @test pwrss(fm1) ≈ 73537.52873209493 atol=0.01 # this quantity is not precisely estimated
    @test stderror(fm1) ≈ [17.694539635084272] atol=0.0001
    ...
    @test objective(fm1) ≈ 319.6542768422537 atol=0.0001
    ...
    @test only(vc) ≈ 375.7170327970783 rtol=1.e-3
...
#        @test last(fitlogtbl.objective) == fm1.optsum.fmin # can't count on this with unconstrained opt
...
@testset "Dyestuff2" begin
    ...
    show(IOBuffer(), fm)
    @test fm.θ ≈ zeros(1)
    @test objective(fm) ≈ 162.87303665382578
    ...
    @test objective(fm) ≈ 327.32705988113764 atol=0.001
...
@testset "penicillin" begin
    ...
    @test objective(fm) ≈ 332.1883486683937 atol=0.001
    @test coef(fm) ≈ [22.972222222225444] atol=0.001
    @test fixef(fm) ≈ [22.972222222225444] atol=0.001
    @test only(coef(fm)) ≈ mean(MixedModels.dataset(:penicillin).diameter)
    @test stderror(fm) ≈ [0.7445940738182094] atol=0.0001
    @test fm.θ ≈ [1.5375935125427782, 3.219745537955045] atol=0.001
    @test first(std(fm)) ≈ [0.8455722686407121] atol=0.0001
    @test std(fm)[2] ≈ [1.7706419263385857] atol=0.0001
    @test varest(fm) ≈ 0.3024254858152094 atol=0.0001
    @test logdet(fm) ≈ 95.74658290779811 atol=0.001

    cv = condVar(fm)
    @test length(cv) == 2
    @test size(first(cv)) == (1, 1, 24)
    @test size(last(cv)) == (1, 1, 6)
    @test first(first(cv)) ≈ 0.0733136032219583 rtol=1.e-4
    @test last(last(cv)) ≈ 0.040515903742453355 rtol=1.e-4

    cv2 = condVar(fm, :sample)
    @test cv2 ≈ last(cv)
    rfu = ranef(fm, uscale=true)
    @test length(rfu) == 2
    @test first(first(rfu)) ≈ 0.5231575862546581 rtol=1.e-4

    rfb = ranef(fm)
    @test length(rfb) == 2
    @test last(last(rfb)) ≈ -3.001823790900832 rtol=1.e-4
...
@testset "pastes" begin
    ...
    @test objective(fm) ≈ 247.99446586223172 atol=0.001
    @test coef(fm) ≈ [60.053333333334] atol=0.001
    @test fixef(fm) ≈ [60.053333333334] atol=0.001
    @test stderror(fm) ≈ [0.642134301364774] atol=0.0001
    @test fm.θ ≈ [3.5269026643032917, 1.329902940085874] atol=0.001
    @test first(std(fm)) ≈ [2.904078953131269] atol=0.001
    @test std(fm)[2] ≈ [1.095052374736776] atol=0.0001
    @test last(std(fm)) ≈ [0.8234077403167984] atol=0.0001
    @test varest(fm) ≈ 0.6780003068136161 atol=0.0001
    @test logdet(fm) ≈ 101.03829418853418 atol=0.001

    cv = condVar(fm)
    @test length(cv) == 2
    @test size(first(cv)) == (1, 1, 30)
    @test first(first(cv)) ≈ 1.1118566326988504 rtol=1.e-4
    @test size(last(cv)) == (1, 1, 10)
    @test last(last(cv)) ≈ 0.8504108947004724 rtol=1.e-4
    ...
    @test first(lrt.tests.pvalues) ≈ 0.5233767963798441 atol=0.0001
...
@testset "InstEval" begin
    ...
    @test objective(fm1) ≈ 237721.76877449563 atol=0.001
    ftd1 = fitted(fm1);
    @test size(ftd1) == (73421, )
    @test ftd1 == predict(fm1)
    @test first(ftd1) ≈ 3.178762112361497 atol=0.0001
    resid1 = residuals(fm1);
    @test size(resid1) == (73421, )
    @test first(resid1) ≈ 1.8212378876385031 atol=0.00001
    ...
    @test objective(fm2) ≈ 237585.55341516886 atol=0.001
...
#    @test lowerbd(fm) == [0.0, -Inf, 0.0]
    ...
    @test objective(fm) ≈ 1751.9393444631903
    @test fm.θ ≈ [0.9292256825071498, 0.01816582794741198, 0.2226453206088903] atol=1.e-6
    @test pwrss(fm) ≈ 117889.38755234834 atol=0.01
    @test logdet(fm) ≈ 73.90333303456077 atol=0.001
    @test stderror(fm) ≈ [6.63227794674162, 1.502236302919122] atol=0.0001
    @test coef(fm) ≈ [251.40510484848605, 10.467285959595777]
    @test fixef(fm) ≈  [251.40510484848605, 10.467285959595777]
    @test std(fm)[1] ≈ [23.78057261257775, 5.716832259222901] atol=0.01
    @test cond(fm) ≈ [4.175261496089308] atol=0.0001
    @test loglikelihood(fm) ≈ -875.9696722315952
    @test sum(leverage(fm)) ≈ 28.611598793702214 rtol=1.e-5
    σs = fm.σs
    @test length(σs) == 1
    @test keys(σs) == (:subj,)
    @test length(σs.subj) == 2
    @test first(values(σs.subj)) ≈ 23.78057261257775 atol=0.0001
    @test last(values(first(σs))) ≈ 5.716832259222901 atol=0.0001
    @test fm.corr ≈ [1.0 -0.13755303228487029; -0.13755303228487029 1.0] atol=0.0001

    u3 = ranef(fm, uscale=true)
    @test length(u3) == 1
    @test size(first(u3)) == (2, 18)
    @test first(first(u3)) ≈ 3.0301510934980853 atol=0.001

    cv = condVar(fm)
    @test length(cv) == 1
    @test size(first(cv)) == (2, 2, 18)
    @test first(first(cv)) ≈ 140.96700808750023 rtol=1.e-4
    @test last(last(cv)) ≈ 5.157777920711596 rtol=1.e-4
    @test first(cv)[2] ≈ -20.604442587333782 rtol=1.e-4
    ...
    @test first(cvtsubjσ1) ≈ 11.872952795640192 atol=1.0e-4
    @test last(cvtsubjσ1) ≈ 2.2710741777211054 atol=1.0e-4
    cvtsubjρ = first(cvtsubj.ρ)
    @test all(==(cvtsubjρ), cvtsubj.ρ)
    @test only(cvtsubjρ) ≈ -0.7641362619433549 atol=1.0e-4

    b3 = ranef(fm)
    @test length(b3) == 1
    @test size(first(b3)) == (2, 18)
    @test first(first(b3)) ≈ 2.8156942179555444 atol=0.001
    ...
    @test objective(fm) ≈ 1751.9393444631903 # check the model is properly restored
    ...
    @test first(only(sigmas)) ≈ 24.17126557107178 atol=1e-4
    ...
    @test deviance(fmnc) ≈ 1752.0032551398779 atol=0.001
    @test objective(fmnc) ≈ 1752.0032551398779 atol=0.001
    @test coef(fmnc) ≈ [251.40510484848542, 10.467285959595731]
    @test fixef(fmnc) ≈ [251.40510484848542, 10.467285959595731]
    @test stderror(fmnc) ≈ [6.707673761706531, 1.5193146333264311] atol=0.001
    @test fmnc.θ ≈ [0.945810709744356, 0.22692826558404863] atol=0.0001
    @test first(std(fmnc)) ≈ [24.17126557107178, 5.7994092438404845] atol=0.001
    @test last(std(fmnc)) ≈ [25.55613435335814] atol=0.001
    @test logdet(fmnc) ≈ 74.46952640773556 atol=0.001
    ...
    @test objective(fmrs) ≈ 1774.0803152805188 rtol=0.00001
    @test fmrs.θ ≈ [0.24353985674150702] rtol=0.00001
...
"""
    ...
    @testset "Cook's Distance" begin
        lme4_cooks = [0.1270714, 0.1267805, 0.243096, 0.0002437091, 0.03145029, 0.2954052, 0.04550505,
                      0.3552723, 0.1984806, 0.4518805, 0.1683441, 0.02902698, 0.004232616, 1.734029e-05,
                      0.003816645, 0.00623334, 0.03219321, 0.05429389, 0.07319191, 0.06649928, 0.007803994,
                      0.001435875, 0.03886176, 0.01013682, 7.076106e-05, 0.02487801, 0.01538649, 0.002299068,
                      0.008366248, 0.08733211, 0.3043884, 0.0770035, 0.003193764, 0.000259058, 0.00841487,
                      0.00664586, 0.0894498, 0.007342141, 0.07721502, 0.00115366, 0.0476889, 0.01107893,
                      0.02342937, 0.04474152, 0.009826393, 0.02536012, 0.07157197, 8.781548e-08, 0.1757661,
                      0.01755979, 0.04308501, 0.04907289, 0.003603381, 0.02141832, 0.01529109, 0.0002237688,
                      1.055383, 0.01226195, 0.01122611, 0.7032865, 0.01801972, 0.008351314, 0.009071886,
                      1.922539e-05, 0.009401271, 0.01932602, 0.0001153177, 0.003751265, 0.02194446, 4.78793e-09,
                      0.02048001, 0.01981013, 0.04247507, 0.03844668, 0.007580713, 0.01639404, 0.001973649,
                      0.006080187, 0.0008513994, 0.08466273, 0.0878464, 0.2161317, 0.0467594, 0.06665132,
                      0.0006486227, 0.0009503809, 0.03397066, 0.1231246, 0.1946271, 0.2816787, 0.008455713,
                      0.02639438, 0.1743106, 0.00450064, 1.73262e-05, 0.01563701, 0.01998501, 0.02539804,
                      0.157366, 0.1206117, 0.002382807, 0.007197368, 0.009506474, 0.002782844, 0.02747835,
                      0.00986326, 0.008074464, 0.001298994, 0.03273043, 0.05191876, 0.005918988, 0.0696993,
                      0.05733613, 0.1038886, 0.0881868, 0.008494316, 0.159206, 0.03677518, 0.135499, 0.06079108,
                      0.003406159, 0.1399327, 0.001825492, 0.00191708, 0.01107303, 0.004549203, 0.02109569,
                      0.1587737, 0.002198379, 0.006746796, 0.3064917, 3.780973e-07, 0.02104387, 0.04698987,
                      0.02207251, 0.009852787, 0.0009590272, 1.506034e-05, 0.001194266, 0.003147009, 0.01284797,
                      1.315739e-05, 0.03073671, 0.00899036, 0.01262709, 0.002494427, 0.03239389, 0.01698841,
                      0.0002320865, 0.0135889, 0.02761053, 0.02916589, 0.04618232, 0.07875934, 0.02248172,
                      0.1308213, 0.04340534, 0.05379937, 0.0873526, 0.07648689, 0.03333461, 0.01267992,
...
@testset "d3" begin
    fm = only(models(:d3))
    @test pwrss(fm) ≈ 5.3047986185787255e6 rtol=1.e-4
    @test objective(fm) ≈ 884957.5539350753 rtol = 1e-4
    @test coef(fm) ≈ [0.4991260407903265, 0.31130777152144157] atol = 1.e-4
    @test length(ranef(fm)) == 3
    @test sum(leverage(fm)) ≈ 8808.015748200247 rtol = 1.e-4
...
@testset "oxide" begin   # the second model is suspect and we only test the first one
