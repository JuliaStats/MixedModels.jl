...
@testset "sleep" begin
    ...
    A11 = view(fm.A, Block(1,1))
    @test isa(A11, UniformBlockDiagonal{Float64})
    @test isa(view(fm.L, Block(1, 1)), UniformBlockDiagonal{Float64})
    @test size(A11) == (36, 36)
    a11 = view(A11.data, :, :, 1)
    @test a11 == [10. 45.; 45. 285.]
    @test size(A11.data, 3) == 18
    λ = first(fm.λ)
    b11 = LowerTriangular(view( view(fm.L, Block(1, 1)).data, :, :, 1))
    @test b11 * b11' ≈ λ'a11*λ + I rtol=1e-5
    @test count(!iszero, Matrix(view(fm.L, Block(1, 1)))) == 18 * 4
    @test rank(fm) == 2
